...
public class Sudoku_Medium extends JFrame implements ActionListener{
	...
	private String[][] keyMedium = new String[][]{
		{"2", "6", "3", "4", "7", "1", "9", "8", "5"},	
		{"8", "4", "9", "5", "2", "6", "7", "1", "3"},
		{"5", "7", "1", "8", "9", "3", "4", "2", "6"},
		{"6", "3", "4", "1", "8", "5", "2", "7", "9"},
	...
	//declares variables to be used below
		...
		public Sudoku_Medium(){
			...
			title = new JLabel("Sudoku: Medium");
				...
				for (x = 50; x <= 900; x+=100){
					for (y = 80; y <= 900; y+=100){
						...
						if(answer == true){//if user chooses to giveup
							g2.drawString(keyMedium[rowCounter][colCounter], x, y);//draws solution to the screen
						}//End of if
						rowCounter++;
					}//End of for
					colCounter++;
			...
			if(e.getActionCommand().equals("Check Solution")){//if user checks the solution
				...
				for (int i = 0; i < 9; i++){
					for (int j = 0; j < 9; j++){
						...
						if(t[i][j].getText().equals(keyMedium[i][j])){//checks to see if each value is equal to the key				
				...
				for (x = 0; x < 9; x++){
					for (y = 0; y < 9; y++){
						...
						t[h][o].setText(keyMedium[h][o]);//sets a few random textbox to the correct value
