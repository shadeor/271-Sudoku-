...
public class Sudoku_Hard extends JFrame implements ActionListener{
	...
	private String[][] keyHard = new String[][]{
		{"3", "4", "9", "1", "2", "8", "7", "6", "5"},	
		{"8", "7", "5", "4", "9", "6", "2", "3", "1"},
		{"1", "6", "2", "5", "7", "3", "8", "9", "4"},
		{"4", "2", "6", "7", "5", "9", "3", "1", "8"},
				...
				for (x = 50; x <= 900; x+=100){
					for (y = 80; y <= 900; y+=100){
						...
						if(answer == true){//if user chooses to giveup
							g2.drawString(keyHard[rowCounter][colCounter], x, y);//draws solution to the screen
						}//End of if
						rowCounter++;
					}//End of for
					colCounter++;
			...
			if(e.getActionCommand().equals("Check Solution")){//if user checks the solution
				...
				for (int i = 0; i < 9; i++){
					for (int j = 0; j < 9; j++){
						...
						if(t[i][j].getText().equals(keyHard[i][j])){//checks to see if each value is equal to the key				
				...
				for (x = 0; x < 9; x++){
					for (y = 0; y < 9; y++){
						...
						t[h][o].setText(keyHard[h][o]);//sets a few random textbox to the correct value
